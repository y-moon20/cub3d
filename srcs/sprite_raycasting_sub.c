...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   sprite_raycasting_sub.c                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: soohchoi <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/12/09 00:57:19 by soohchoi          #+#    #+#             */
/*   Updated: 2020/12/09 01:11:23 by soohchoi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_spr_ray *s_ray, int i)
{
	...
	v_mode = 0.0;
	s_ray->x = info->spr[i].x - player->x;
	s_ray->y = info->spr[i].y - player->y;
	...
	s_ray->screen_x = (int)((info->width / 2) * (1 + s_ray->transform_x /\
				s_ray->transform_y));
...
void	calculate_sprite_height(t_info *info, t_spr_ray *s_ray)
{
	...
	s_ray->draw_start_y = -s_ray->height / 2 + info->height / 2 +\
						s_ray->v_move_screen;
	if (s_ray->draw_start_y < 0)
		s_ray->draw_start_y = 0;
	s_ray->draw_end_y = s_ray->height / 2 + info->height / 2 +\
						s_ray->v_move_screen;
...
void	get_sprite_color(t_info *info, t_spr_ray *s_ray, int stripe)
{
	...
	s_ray->tex_x = (int)((256 * (stripe - (-s_ray->width / 2 +\
						s_ray->screen_x)) * TEX_WIDTH / s_ray->width) / 256);
	if (s_ray->transform_y > 0 && stripe > 0 && stripe < info->width &&\
			s_ray->transform_y < info->z_buffer[stripe])
	{
		y = s_ray->draw_start_y;
		while (y < s_ray->draw_end_y)
		{
			draw = (y - s_ray->v_move_screen) * 256 - info->height *\
				128 + s_ray->height * 128;
			s_ray->tex_y = ((draw * TEX_HEIGHT) / s_ray->height) / 256;
			color = info->tex[4][TEX_WIDTH * s_ray->tex_y + s_ray->tex_x];
